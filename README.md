...
# Step 4: Classification Approaches

This step involves two main classification phases:
1. Initial classification using DictaLM 2.0 model to establish ground truth
2. Predictive modeling using various BERT-based approaches to predict harmful language before it occurs

## Phase 1: Initial Classification Using DictaLM
...
The first script (`process_conversations.py`) identifies disrespectful language in conversations by:
- Analyzing conversations around formal order calls
- Examining a window of ±3 interactions around each order call
- Classifying responses as respectful or disrespectful
...
The second script (`offensive_language.py`) specifically focuses on detecting offensive language by:
- Processing batches of conversations
- Using a specialized lexicon of offensive words
- Handling special cases like the word "נוכל" (can be either offensive or neutral based on context)
...
The code implements several optimization techniques:
- Batch processing to manage memory usage
- Periodic cache clearing using `torch.cuda.empty_cache()`
- Garbage collection using `gc.collect()`
- Progress tracking using `tqdm`

## Output Files

The classification produces three types of output files:

1. **Positive Classifications** (`*_yes.csv`)
   - Contains conversations classified as disrespectful/offensive
   - Includes file name, conversation text, and model response

2. **Negative Classifications** (`*_no.csv`)
   - Contains conversations classified as respectful/non-offensive
   - Same structure as positive classifications

3. **Updated Original Dataset** (`*_all.csv` or `*_df.csv`)
   - Original dataset with added classification column
   - Includes all metadata and classification results

## Usage

1. **For Disrespectful Language Classification:**
```python
...
2. **For Offensive Language Classification:**
```python
# Set batch number and paths
batch_num = 4
batch_path = f"path/to/batch_{batch_num}_offensive.csv"
df = pd.read_csv(batch_path)

# Run classification
# The script will automatically process the dataframe and save results
```
